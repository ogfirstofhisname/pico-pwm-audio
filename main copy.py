...
class DMAWavPlayer:
    ...
    def play_wav(self, wav_file_path:str=None, loop:bool=None) -> None:
        ...
        while True: # playback loop (every iteration is a full file playback)
            ...
            print('Starting playback chunk loop')            
            while got_bytes > 0: # playback chunks loop
                print(f'playback chunk loop: got_bytes = {got_bytes}')
    ...
    def _preload_buffers(self):
        ...
        got_bytes = self._fill_buffer_from_wav()
        self._convert_buffer_to_pio_samples()
        print(f'len of lhs: {len(self.third_buffer[:self.buffer_n_samples])}, len of rhs: {len(self.pio_samples_buffer)}')
...
class RingDMAWavPlayer(DMAWavPlayer):
    ...
    def _allocate_third_buffer(self):
        ...
        print(f'buffer_n_samples: {self.buffer_n_samples}, large buffer size: {len(self._large_buffer)} samples ({len(self._large_buffer) * 2} bytes).')
        ...
        print(f'Base address: {hex(base_address)}, align size: {hex(align)} bytes, start address: {hex(start)}, offset: {hex(offset)} bytes.')
        ...
        print(f'created excessively large buffer of size {len(self._large_buffer)} samples ({len(self._large_buffer) * 2} bytes), base address: {hex(base_address)}.')
        print(f'Aligned third buffer start address: {hex(self._third_buffer_start_address)}, size: {self._third_buffer_size_bytes} bytes.')
    ...
    def _set_up_dma(self):
        ...
        print(f'calculated ring size: {ring_size} bits')
